...
class PICKModel(nn.Module):
    ...
    def forward(self, validation=False, **kwargs):
        # input
        ...
        iob_tags_label = kwargs['iob_tags_label'] if self.training or validation else None  # (B, N, T)
        # CHANGED SELF TRAINING
        ...
        # (B, N, D), (B, N, N), (B,)
        x_gcn, soft_adj, gl_loss = self.graph(x_gcn, relation_features, init_adj, boxes_num, validation)
        adj = soft_adj * init_adj

        ### Decoder module ###
        logits, new_mask, log_likelihood = self.decoder(x.reshape(B, N, T, -1), x_gcn, mask, text_length,
                                                        iob_tags_label, validation)
        ##### Forward End #####
