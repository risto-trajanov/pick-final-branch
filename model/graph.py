...
class GraphLearningLayer(nn.Module):
    ...
    def forward(self, x: Tensor, adj: Tensor, box_num: Tensor = None, validation=False):
        '''
...
class GLCN(nn.Module):
    ...
    def forward(self, x: Tensor, rel_features: Tensor, adj: Tensor, box_num: Tensor, validation=False, **kwargs):
        '''
        ...
        soft_adj, gl_loss = self.gl_layer(x, adj, box_num, validation)
        adj = adj * soft_adj
