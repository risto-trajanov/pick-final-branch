    ...
    def _train_epoch(self, epoch):
        ...
        for step_idx, input_data_item in enumerate(self.data_loader):
            ...
            if self.config['trainer']['anomaly_detection']:
                ...
                # model forward
                output = self.model(**input_data_item)
                # print(f'Output for batch num {step_idx}:{output}')
                # calculate loss
                gl_loss = output['gl_loss']
                crf_loss = output['crf_loss']
                total_loss = torch.sum(crf_loss) + self.gl_loss_lambda * torch.sum(gl_loss)
    ...
    def _valid_epoch(self, epoch):
        ...
        with torch.no_grad():
            for step_idx, input_data_item in enumerate(self.valid_data_loader):
                ...
                output = self.model(**input_data_item, validation=True)
                gl_loss_val = output['gl_loss']
                crf_loss_val = output['crf_loss']
                # Use a barrier() to make sure that all process have finished forward and backward
                if self.distributed:
                    dist.barrier()
                    #  obtain the sum of all total_loss at all processes
                    ...
                    size = 1
                gl_loss_val /= size  # averages gl_loss across the whole world
                crf_loss_val /= size  # averages crf_loss across the whole world

                # calculate average loss across the batch size
                avg_gl_loss_val = torch.mean(gl_loss_val)
                avg_crf_loss_val = torch.mean(crf_loss_val)
                avg_loss_val = avg_crf_loss_val + self.gl_loss_lambda * avg_gl_loss_val
                ...
                # print(f'Best paths for validation batch {step_idx}:{best_paths}')
                ...
                self.writer.add_scalar('loss',avg_loss_val.item())
                self.writer.add_scalar('crf_loss',avg_crf_loss_val.item())
                self.writer.add_scalar('gl_loss',avg_gl_loss_val.item()*self.gl_loss_lambda)
                ...
                # print("Golden tags: ",golden_tags)
                mask = input_data_item['mask']
                union_iob_tags = iob_tags_to_union_iob_tags(golden_tags, mask)
                # print("Union iob tags: ", union_iob_tags)
                if self.distributed:
                    dist.barrier()  #
        ...
        # print("F1 dictionary: ", f1_result_dict)
